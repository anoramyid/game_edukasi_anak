<!--
  TSA-X v6.3 — AI Engineering OS (+ Function Scope-Lock + Exec Cache)
...
FUNCTION_SCOPE_LOCK    only modify the exact function(s) named in prompt scope
                       other functions in same file = read-only context, never touched
                       if no function named explicitly → write AMBIGUITY, do not guess
SCOPE_UNIT             generalizes FUNCTION_SCOPE_LOCK to any named unit:
                       function / method / query / config-key / CSS-rule / route / field
                       whatever unit is explicitly named = editable; everything else
                       in that file stays read-only, regardless of file type
CACHE_BEFORE_EDIT      snapshot target function's body (pre-edit) before any change
                       enables rollback without re-scanning file if validation fails
CACHE_LITE             snapshot = diff or (hash + line-range), not full body,
                       unless target unit is ≤15 lines → then full body is fine
                       goal: rollback capability without paying full-body token cost
LOG_ON_SUCCESS         every validated execution → EXEC_OK entry in memory.md [LOG]
                       success is logged same as errors/decisions — not silently skipped
BATCH_LOG              mode 2/3 touching multiple units in one file →
                       one EXEC_OK entry per file (list units inside), not one per unit
CACHE_TTL              cache/EXEC_OK entries older than 10 sessions with no rollback use
                       → collapse into one summary line in [LOG], details dropped
MAX_LOG_SIZE           if memory.md [LOG] exceeds ~150 lines → trigger CACHE_TTL rollup
                       before continuing work, don't wait for manual cleanup
```
...
✗  Modify a function not explicitly named in prompt scope
✗  Edit without caching pre-edit snapshot of target function
✗  Store full-body cache for units larger than 15 lines (use diff/hash instead)
✗  Let memory.md [LOG] grow past ~150 lines without rollup
```
...
└─► MODE 1 — PATCH
    ...
    Scope: only the function(s) explicitly named in prompt
    Output: audit → cache snapshot → patch → validation → final report
...
Categories: `FILE_EDIT` `DIR_CREATE` `COMMAND` `DECISION` `ERROR` `NOTE` `RESOLVED` `DEBUG` `EXEC_OK`

`EXEC_OK` — validated execution succeeded, scoped to named unit(s):
```
EXEC_OK [YYYY-MM-DD HH:MM] — [file] : [unit_1, unit_2, ...]
Cache : [diff ref or hash+line-range per unit] (full body only if ≤15 lines)
Validated: [how it was verified]
```
...
✓  RATE_LIMIT_FROM_SOURCE — if user provided a rate-limit table/screenshot/dashboard
   ...
   If model/tier not in the stored table → fall back to NEVER_ASSUME rule above.
```
